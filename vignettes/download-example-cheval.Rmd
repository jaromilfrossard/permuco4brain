...
In the following tutorial, we download EEG data analysed in @cheval. This data-set is stored in the [zenodo repository](https://zenodo.org/record/1169140) as "edf" files.

## Installing packages

We need several packages including `tidyverse`  [@tidyverse], `edf` [@edf], `abind` [@abind], `xlsx` [@xlsx] and `permuco4brain` [@frossard_permuco4brain]:
...
We download the data in a temporary files and we also unzip the `.edf` files in a temporary file.
...
We download information on the subject and we only keep the centred measure of "Moderate to Vigorous Physical Activity" (MVPA) as between subjet covariates:
...
We create the design from the filenames and we join the "between"-subject data. Moreover, we keep only the `"Round"` stimuli as neutral shapes.
...
We create the data-frame that stores the design:
...
The 3 objects `signal`, `design`, `graph` are saved for future uses:

```{r save, echo=TRUE,eval=FALSE}
# save(signal,design,graph,file = "signal_design_graph.RData")
```

Or we simply run the permutation tests:
