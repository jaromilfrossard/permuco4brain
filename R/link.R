...
link = function(formula_f,formula_within){
  ...
  factors_f = attr(t_f, "factors")
  factors_between = colSums(factors_f[rownames(factors_f) %in%
                                        attr(t_within, "term.labels"), , drop = F]) == 0
  factors_link = factors_f
  factors_link[!rownames(factors_f) %in% attr(t_within, "term.labels"),
               ] = 0
  ...
  link = rbind(order = attr(t_f, "order"), between_effect = factors_between *
                 c(1:length(factors_between)), within = factors_link)
  link
}
